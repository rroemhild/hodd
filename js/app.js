...
// MQTT Client
var clientId = "hodd_" + Math.random().toString(16).substr(2, 8)
  ...
  if (!(device_id in devices.deviceList)) {
    ...
    if (topic[2] === "$name") {
      ...
      if (topic[3] === "$name") {
        ...
        for (p in properties) {
          devices.$set(devices.deviceList[device_id].nodes[node].properties, properties[p], {
            id: properties[p],
            name: "",
            settable: "false",
            datatype: "string",
          ...
          client.subscribe(topic[0] + "/" + topic[1] + "/" + topic[2] + "/" + properties[p])
          client.subscribe(topic[0] + "/" + topic[1] + "/" + topic[2] + "/" + properties[p] + "/+");
        }
      }
      else if (topic[3] in devices.deviceList[device_id]["nodes"][node]["properties"]) {
        property = topic[3]
        if (topic.length === 4) {
          devices.deviceList[device_id]["nodes"][node]["properties"][property]["state"] = payload;
          devices.deviceList[device_id]["nodes"][node]["properties"][property]["topic"] = message.destinationName
        }
        else if (topic[4] === "$name") {
          devices.deviceList[device_id]["nodes"][node]["properties"][property]["name"] = payload;

        }
        else if (topic[4] === "$settable") {
          devices.deviceList[device_id]["nodes"][node]["properties"][property]["settable"] = payload;
        }
        else if (topic[4] === "$datatype") {
          devices.deviceList[device_id]["nodes"][node]["properties"][property]["datatype"] = payload;
        }
        else if (topic[4] === "$format") {
          devices.deviceList[device_id]["nodes"][node]["properties"][property]["format"] = payload;
        }
        else if (topic[4] === "$retained") {
          devices.deviceList[device_id]["nodes"][node]["properties"][property]["retained"] = payload;
        }
        else if (topic[4] === "$unit") {
          devices.deviceList[device_id]["nodes"][node]["properties"][property]["unit"] = payload;
        }
      }
    }
  }
